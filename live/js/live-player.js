(function(){
  ...
  function contactOffset(playerIndex,contact){
    ...
    const atNet=playerIndex===0?contact.y>35:contact.y<65;
    return{
      x:Math.max(13,Math.min(87,contact.x+lateral)),
      y:Math.max(playerIndex===0?8:52,Math.min(playerIndex===0?48:92,contact.y+(playerIndex===0?-1.8:1.8))),
      atNet
    };
  }
  ...
  function recoveryPosition(playerIndex,from){
    ...
    const netBias=playerIndex===0?38:62;
    const wasAtNet=playerIndex===0?from.y>35:from.y<65;
    return{
      x:home.x+(from.x-home.x)*.34,
      y:wasAtNet ? netBias : home.y+(from.y-home.y)*.44
    };
  }

  function fallbackServeLanding(serverIndex){
    return{x:serverIndex===0?65:35,y:serverIndex===0?61:39};
  }

  function fallbackReceiveContact(serverIndex,landing){
    return{x:landing.x,y:serverIndex===0?77:23};
  }

  function fallbackShotLanding(hitter,target){
    const x={left:28,center:50,right:72}[target]||50;
    return{x,y:hitter===0?79:21};
  }

  function setupPoint(e,duration){
    state.server=e.server;
    state.serveSide=e.serveSide;
    const serverPos=e.serverPosition||COURT_POSITIONS.neutral[e.server];
    const receiverPos=e.receiverPosition||COURT_POSITIONS.returnBase[e.receiver];
    const travel=Math.max(360,Math.min(760,duration*.82));
    movePlayer(e.server,serverPos.x,serverPos.y,travel,'run');
    movePlayer(e.receiver,receiverPos.x,receiverPos.y,travel,'run');
    later(()=>setBall(serverPos.x+(e.server===0?2:-2),serverPos.y+(e.server===0?3:-3)),travel*.72);
  }

  function playServe(e,duration){
    const server=e.player,receiver=1-server;
    const pos=e.serverPosition||positions.players[server];
    const landing=e.landing||e.visualTarget||fallbackServeLanding(server);
    const receive=e.receiveContact||fallbackReceiveContact(server,landing);
    const toss=Math.max(120,Math.min(190,duration*.18));
    const firstFlight=Math.max(220,duration*.38);
    const bounceFlight=Math.max(150,duration*.25);
    const contactY=pos.y+(server===0?-4:4);

    setBall(pos.x+(server===0?2:-2),pos.y+(server===0?3:-3));
    animateBallTo(pos.x,contactY,toss,true);

    later(()=>{
      animateRacket(server,'serve-motion');
      showContact(pos.x,contactY,server);
      animateBallTo(landing.x,landing.y,firstFlight,true);
      const receiverApproach=contactOffset(receiver,receive);
      movePlayer(receiver,receiverApproach.x,receiverApproach.y,firstFlight+bounceFlight,'run');
      const recover=recoveryPosition(server,pos);
      movePlayer(server,recover.x,recover.y,firstFlight+bounceFlight,'recover');

      later(()=>{
        showBounce(landing.x,landing.y);
        animateBallTo(receive.x,receive.y,bounceFlight,false);
      },firstFlight);
    },toss);
  }
  ...
  function playShot(e,duration){
    ...
    const landing=e.landing||e.visualTarget||fallbackShotLanding(hitter,e.target);
    const receive=e.receiveContact||landing;
    const hasBounce=e.bounce!==false;
    const finalStep=Math.max(90,Math.min(210,duration*.18));
    const hold=Math.max(40,Math.min(75,duration*.06));
    const firstFlight=Math.max(210,duration*(hasBounce?.38:.56));
    const bounceFlight=hasBounce?Math.max(140,duration*.24):0;
    const hitterPos=contactOffset(hitter,contact);

    movePlayer(hitter,hitterPos.x,hitterPos.y,finalStep,'run');
    animateBallTo(contact.x,contact.y,finalStep*.86,false);

    later(()=>{
      setBall(contact.x,contact.y);
      showContact(contact.x,contact.y,hitter);
      animateRacket(hitter,hitterPos.atNet?'volley':'swing');
    },finalStep);

    later(()=>{
      animateBallTo(landing.x,landing.y,firstFlight,false);

      if(e.isFinal&&e.finishType==='winner'&&e.pointWinner===hitter){
        const miss=e.defenderTarget||{
          x:receive.x<50?Math.min(84,receive.x+25):Math.max(16,receive.x-25),
          y:opponent===0?22:78
        };
        movePlayer(opponent,miss.x,miss.y,firstFlight+(hasBounce?bounceFlight:0),'run');
      }else if(e.isFinal&&e.finishType==='unforced_error'){
        const ready=recoveryPosition(opponent,positions.players[opponent]);
        movePlayer(opponent,ready.x,ready.y,firstFlight*.8,'anticipate');
      }else{
        const approach=contactOffset(opponent,receive);
        movePlayer(opponent,approach.x,approach.y,firstFlight+(hasBounce?bounceFlight:0),'run');
      }
      ...
      later(()=>movePlayer(hitter,recover.x,recover.y,firstFlight*.72,'recover'),Math.max(70,firstFlight*.15));
    },finalStep+hold);
  }
  ...
  function settleAfterPoint(winner){
    ...
    movePlayer(winner,winnerHome.x,winnerHome.y,420,'recover');
    movePlayer(loser,loserHome.x,loserHome.y,420,'recover');
  }
  ...
  function playEvent(e,duration){
    switch(e.type){
      ...
      case 'point_start':
        ...
        state.serveSide=e.serveSide;
        renderProgress('—');
        $('currentPhase').textContent=e.serveSide==='deuce'?'Подача слева → вправо':'Подача справа → влево';
        $('status').textContent=`${playerName(e.server)} занимает позицию для следующей подачи.`;
        setupPoint(e,duration);
        break;
      case 'serve':
        $('currentPhase').textContent=e.firstServe?'Первая подача':'Вторая подача';
        $('status').textContent=`${playerName(e.player)} подаёт по диагонали — ${e.speed} км/ч${e.firstServe?'':' (вторая)'}.`;
  ...
  function applySnapshotEvent(e,messages){
    switch(e.type){
      ...
      case 'point_start':
        state.server=e.server;
        state.serveSide=e.serveSide;
        positions.players[e.server]={...(e.serverPosition||COURT_POSITIONS.neutral[e.server])};
        positions.players[e.receiver]={...(e.receiverPosition||COURT_POSITIONS.returnBase[e.receiver])};
        positions.ball={
          x:positions.players[e.server].x+(e.server===0?2:-2),
          y:positions.players[e.server].y+(e.server===0?3:-3)
        };
        break;
      case 'serve': {
        ...
        const pos=e.serverPosition||positions.players[server];
        const landing=e.landing||fallbackServeLanding(server);
        const receive=e.receiveContact||fallbackReceiveContact(server,landing);
        positions.players[server]=recoveryPosition(server,pos);
        positions.players[receiver]=contactOffset(receiver,receive);
        positions.ball={...receive};
        break;
      }
      case 'shot': {
        ...
        const contact=e.contact||positions.ball;
        const landing=e.landing||fallbackShotLanding(hitter,e.target);
        const receive=e.receiveContact||landing;
        ...
        if(e.isFinal && e.finishType==='winner' && e.pointWinner===hitter){
          ...
          positions.players[opponent]=contactOffset(opponent,receive);
        }
        positions.ball={...receive};
        break;
      }
